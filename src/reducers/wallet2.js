const DEFAULT_STATE = { currencies: ['BRL'], expenses: [], total: 0, apiResponse: ['BRL'], fetched: false, id: 0, latesteCurrency: '', cashList: [] };
...
const wallet2 = (state = DEFAULT_STATE, action) => {
  ...
  case 'Item':
    return {
      ...
      cashList: [...state.cashList, (Number(action.payload.value)) * (Number(action.payload.exchangeRates[action.payload.currency].ask))],
